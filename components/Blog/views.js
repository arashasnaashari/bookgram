...
const Compo = ({ userId, bookId, img, name }) => {
  ...
  const { data } = useSWR(`/api/viewss/${bookId}/0/7`, fetcher);
  const handleCreateComnt = async (e) => {
    ...
    if (!userGot) {
      ...
      const res = await fetch("http://localhost:3000/api/createview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, userId, bookId }),
      });
      const data1 = await res.json();
      mutate(`/api/viewss/${bookId}/0/7`);
    }
  };
  return (
    <>
        ...
        {userId && (
          ...
          >
            <textarea
              ...
              placeholder="نظر خود رو درمورد نقد بنویس .... "
        ...
        {data &&
          data.map((e) => {
            return (
              <>
                <div
                  className="flex flex-col bg-white border-gray-200 border-2 rounded-md p-4 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 font-Vazir md:w-3/4"
                  style={{ direction: "rtl" }}
                >
                                  ...
                                  onSubmit={async (s) => {
                                    ...
                                    if (!userGot) {
                                      ...
                                      const res = await fetch(
                                        "http://localhost:3000/api/replyview",
                                        {
                                          method: "POST",
                                          headers: {
                        ...
                        {userId == e.creator._id && (
                          <button
                            onClick={async (s) => {
                              ...
                              const res = await fetch(
                                "http://localhost:3000/api/deleteview",
                                {
                                  method: "POST",
                                  headers: {
                                    "Content-Type": "application/json",
